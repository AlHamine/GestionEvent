...
function DemandeList() {
  ...
  const contrat = () => {
    ...
    fetch(
      SERVER_URL + `demandes/contrat/${sessionStorage.getItem("UserMail")}`,
      {
    ...
    fetch(SERVER_URL + `api/demandes/${demande.idDemande}`, {
      ...
      .then((response) => {
        if (response.ok) {
          ...
          fetch(
            SERVER_URL +
              `prestataires/event/${demande.prestataire.idp}/${demande.evenement.idEvent}`,
            {
              method: "PUT",
              headers: {
                ...
                {d.status === "EN_ATTENTE" ? (
                  <div>
                    <Button
                      variant="contained"
                      color="success"
                      onClick={() => repondreDemande(d, "ACCEPTED")}
                    >
                      Accepter
                    </Button>
                    <Button
                      variant="contained"
                      color="error"
                      onClick={() => repondreDemande(d, "REFUSED")}
                    >
                      Refuser
                    </Button>
                  </div>
                    ...
                    {d.status === "ACCEPTED" ? (
                      ...
                      <div>
                        ...
                        <CardMedia
                          ...
                          height="200"
                          image={
                            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSs5yGvr5YTvswccJ2KXUI3HOYQEEOKqLlM9A&usqp=CAU  "
                          }
                          alt={d.idDemnde}
                        />
                      </div>
