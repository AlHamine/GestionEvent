...
public interface MessageRepository extends CrudRepository<Message, Long> {
    ...
    @Query(value = "SELECT d.* FROM message d WHERE d.source= :m ", nativeQuery = true)
    List<Message> mesMessage(@Param("m") String mail);

        @Query(value = "SELECT d.* FROM message d WHERE d.source= :m and d.source= :des ", nativeQuery = true)
